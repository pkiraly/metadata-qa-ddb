...
mysql --defaults-extra-file=$ROOT/mysql-config.cnf $MY_DB  -e "SELECT f.metadata_schema, f.set_id, f.provider_id, f.file, i.*
  FROM issue AS i
...
echo variability
mysql --defaults-extra-file=$ROOT/mysql-config.cnf $MY_DB -e "DELETE FROM variability;"
php $ROOT/scripts/csv2sql.php ${OUTPUT_DIR}/variability.csv variability
mysql --defaults-extra-file=$ROOT/mysql-config.cnf $MY_DB < ${OUTPUT_DIR}/variability.sql
...
echo count
mysql --defaults-extra-file=$ROOT/mysql-config.cnf $MY_DB -e "DELETE FROM count;"
php $ROOT/scripts/csv2sql.php ${OUTPUT_DIR}/count.csv count
mysql --defaults-extra-file=$ROOT/mysql-config.cnf $MY_DB < ${OUTPUT_DIR}/count.sql
